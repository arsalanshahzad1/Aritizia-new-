                        ...
                        <div className='controls-for-user-management'>
                            <div className='user-sub-dd' onClick={() => setShowFilter(!showfilter)}>
                                ...
                                <div className="options">
                                    <h2 onClick={() => { viewUserList(+(list?.pagination?.page), 'yearly', searchInput); setFilter("yearly") }}>Yearly</h2>
                                    <h2 onClick={() => { viewUserList(+(list?.pagination?.page), 'monthly', searchInput); setFilter("monthly") }}>Monthly</h2>
                                    <h2 onClick={() => { viewUserList(+(list?.pagination?.page), 'weakly', searchInput); setFilter("weekly") }}>Weekly</h2>
                                </div>
                                } 
                            </div>
                ...
                <div className='table-for-user-management'>
                    ...
                    <div className='user-management-table-contorls'>
                        <div>
                            ...
                            <div>of Pages</div>
                            <div onClick={() => { viewUserList(+(list?.pagination?.page) - 1 , filter , searchInput); setIsOpen(null) }} className={`${list?.pagination?.page < 2 ? 'disable' : ''}`}>
                            ...
                            <div onClick={() => { viewUserList(+(list?.pagination?.page) + 1 , filter , searchInput);; setIsOpen(null) }} className={`${list?.pagination?.remaining == 0 ? 'disable' : ''}`}>
                                <svg width="7" height="13" viewBox="0 0 7 13" fill="none" xmlns="http://www.w3.org/2000/svg">
