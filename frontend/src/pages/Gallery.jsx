...
const Gallery = ({ user, loader, setLoader }) => {
    ...
    const [selectedGalleryCollection, setSelectedGalleryCollection] =
        useState({ value: { id: '', name: '' }, options: [] })
    ...
    useEffect(() => {
        var totalCount = folderGalleryItems.reduce(function (accumulator, folder) {
            var selectedImagesCount = folder.images.reduce(function (innerAccumulator, image) {
    ...
    const saveToGallery = async () => {
        ...
        try {
            ...
            for (let index = 0; index < response?.data?.data?.folders.length; index++) {
                tempFolderImages = []

                for (let j = 0; j < response?.data?.data?.folders?.[index]?.images?.length; j++) {
    ...
    const listFolders = async () => {
        ...
        try {
            setSelectedGalleryCollection((prev) => ({ ...prev, options: response?.data?.data }))
        } catch (error) {
            console.log(error);
        }
    }
    ...
    const storeFolder = async () => {
        if (foldername === '') {
            ...
            try {
                setSelectedGalleryCollection((prev) => ({ ...prev, value: { id: response?.data?.data?.id, name: response?.data?.data?.title } }))
    ...
    const copyAndMoveMethodeFolder = async () => {
        ...
        if (transferType === 'move') {
            ...
            try {
                ...
                setSelectedGalleryCollection((prev) => ({ ...prev, value: { id: '', name: '' } }))
            } catch (error) {
                toast.error(error?.message);
            }
    ...
    const copyAndMoveMethode = async () => {
        if (transferType === 'move') {
            var selectedIds = allGalleryItems.filter(obj => obj.selected === true).map(obj => obj.id);
        ...
        <>
                ...
                {user === "admin" ?
                    (selectFolderCount === 0 &&
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                            ...
                            <button
                                disabled={+fullGalleryResponse?.pagination?.page === fullGalleryResponse?.pagination?.total_pages}
                                onClick={() => loadmoreGallaryImages(+fullGalleryResponse?.pagination?.page + 1)}
                                className='save-art-btn'>Load More</button>


                            <button onClick={() => { setShowDeleteConfirmation(true); setSelectedFolderId(0) }} disabled={selectCount < 1} className='save-art-btn'>Delete</button>


                            <button onClick={() => { setAddToCollection(true);; setSelectedFolderId(0) }} disabled={selectCount < 1} className='save-art-btn'>Add into Collection ({selectCount})</button>
                        ...
                        return (
                            <div className="image-collection-folder-section" key={Index}>
                                <div className="sec-title">
                                    <h3>{item.title}</h3> <span onClick={() => { setShowDeleteFolderConfirmation(true); setSelectedFolderId(item?.id) }}><MdDeleteOutline /></span>
                                </div>
                                        ...
                                        {user === "admin" ?
                                            (selectCount === 0 &&
                                                (
                                                    ...
                                                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                                                        {accountAddress !== "false" ?
                                                            <button onClick={ListFolderGallery} disabled={selectFolderCount < 1} className='save-art-btn'>Listing ({selectFolderCount})</button>
                                                            :
                                                            <button onClick={() => setConnectPopup(true)} disabled={selectFolderCount < 1} className='save-art-btn'>Listing ({selectFolderCount})</button>
                                                        }

                                                        <button 
                                                        disabled={+fullGalleryResponse?.folders?.[Index]?.pagination?.page === fullGalleryResponse?.folders?.[Index]?.pagination?.total_pages} 
                                                        onClick={() => loadmoreFolderImages(+fullGalleryResponse?.folders?.[Index]?.pagination?.page + 1 , item?.id , Index , item?.title)}
                                                        className='save-art-btn'
                                                        >Load More</button>


                                                        <button onClick={() => { setShowDeleteConfirmation(true); setSelectedFolderId(item?.id) }} disabled={selectFolderCount < 1} className='save-art-btn'>Delete</button>

                                                        <button onClick={() => { setAddToFolderCollection(true); setSelectedFolderId(item?.id) }} disabled={selectFolderCount < 1} className='save-art-btn'>Add into Collection ({selectFolderCount})</button>
                    ...
                    <div className="main-data">
                        ...
                        <div className="body">
                            ...
                            {createAndOld === 'create_new_collection' &&
                                <input type="text" onChange={(e) => setFoldername(e.target.value)} name="" placeholder='Type Collection Name' id="" />
                            }
                                        ...
                                        <ul className='option'>
                                            {selectedGalleryCollection?.options.map((item, index) => {
                                                return (
                                                    <li
                                                        key={index}
                                                        onClick={() => { setSelectedGalleryCollection((prev) => ({ ...prev, value: { id: item?.id, name: item?.title } })) }}>{item?.title}</li>
                                                )
                                            })}
                                        </ul>
                                    }
                            ...
                            <div className="button">
                                {createAndOld === 'create_new_collection' ?
                                    <button className={`${foldername.length < 3 ? 'disable' : ''}`} onClick={storeFolder}>Next</button>
                                    :
                                    <button className={`${(createAndOld == '' || selectedGalleryCollection?.value?.name === '') ? 'disable' : ''}`} onClick={() => { setAddToCollection(false), setShowTransferType(true) }}>Next</button>

                                }
                                <button onClick={() => { setAddToCollection(false) }}>Cancle</button>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="body">
                            ...
                            {createAndOld === 'existing_collection' &&
                                <div className='dd' onClick={() => { setShowCollectionDropdown(!showCollectionDropdown) }}>
                                    <p>{selectedGalleryCollection?.value?.name === '' ? 'Select Collection' : selectedGalleryCollection?.value?.name} <IoIosArrowDown /></p>
                                    {
                                        ...
                                        <ul className='option'>
                                            <li
                                                onClick={() => { setSelectedGalleryCollection((prev) => ({ ...prev, value: { id: 0, name: 'Main Gallery' } })) }}>Main Gallery</li>
                                            {selectedGalleryCollection?.options.map((item, index) => {
                                                return (
                                                    <li
                                                        key={index}
                            ...
                            <div className="button">
                                {createAndOld === 'create_new_collection' ?
                                    <button className={`${foldername.length < 3 ? 'disable' : ''}`} onClick={storeFolder}>Next</button>
                                    :
                                    <button className={`${(createAndOld == '' || selectedGalleryCollection?.value?.name === '') ? 'disable' : ''}`} onClick={() => { setAddToFolderCollection(false), setShowTransferType(true) }}>Next</button>

                                }
                                <button onClick={() => { setAddToFolderCollection(false) }}>Cancle</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="modal-body">
                    <div className="main-data">
                        ...
                        <div className="body">
                            ...
                            <div className="button">
                                {selectedFolderId === 0 ?
                                    <button onClick={copyAndMoveMethode} className={`${transferType == '' ? 'disable' : ''}`}>Done</button>
                                    :
                                    <button onClick={copyAndMoveMethodeFolder} className={`${transferType == '' ? 'disable' : ''}`}>Done</button>
                                }
                                <button onClick={() => { setShowTransferType(false) }}>Cancle</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            >
                <div className="modal-body">
                    <div className="main-data">
                        <div className="header-connect-close">
                            <AiOutlineClose onClick={() => setShowDeleteFolderConfirmation(false)} />
                        </div>
                        <div className="body">
                            <p>Are you sure?</p>
                            ...
                            <div className="button">
                                ...
                                <button onClick={() => setShowDeleteFolderConfirmation(false)}>Cancel</button>
                            </div>
                        </div>
                    </div>
                </div>
